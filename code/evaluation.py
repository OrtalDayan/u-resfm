...
def softmax_inverse_per_column(errors, temperature=1.0, epsilon=1e-6):
    """
    Compute softmax(1/errors) independently for each column.
    
    Args:
        errors: Tensor of shape [num_samples, num_tracks] 
                Each column represents errors for one track
        temperature: Temperature for softmax (lower = sharper distribution)
        epsilon: Small value to prevent division by zero
        
    Returns:
        weights: Tensor of same shape with softmax weights per column
                 Each column sums to 1
    """
    # Compute importance scores (1/error)
    importance = 1.0 / (errors + epsilon)  # Shape: [num_samples, num_tracks]
    
    # Apply temperature scaling
    importance = importance / temperature
    
    # Apply softmax along dimension 0 (per column)
    weights = softmax(importance, axis=0)
...
def compute_errors(outputs, conf, bundle_adjustment, compute_rep_errs=False):
    ...
    rep_errors_like_M = geo_utils.reprojection_error_with_points(Ps, pts3D_pred.T, xs)
   
    if compute_rep_errs: 
        
        stage = conf.get_string('general.stage')
        if stage == "1":
          
            try:
                proj_err_path = conf.get_string('results_path')
                
                proj_err_dir = os.path.dirname(proj_err_path)
                if not os.path.exists(proj_err_dir):
                    os.makedirs(proj_err_dir)
                    print(f"Created directory: {proj_err_dir}")
                   
                # Save the reprojection errors
                write_results(conf, rep_errors_like_M, file_name="reprojection_errors", append=False)
                print("Saved reprojection errors")
            except Exception as e:
                prints(f"ERROR: Failed to save reprojection errors: {e}")
                import traceback
                traceback.print_exc()
