...
#     Args:
#         reconstruction (pycolmap.Reconstruction): The COLMAP reconstruction object.
#         original_num_points (int, optional): Original number of points. If provided,
#                                             returns array of this size with NaN for filtered points.
...
#         Ps (np.ndarray): [N, 3, 4] Projection matrices.
#         Ks (np.ndarray): [N, 3, 3] Intrinsic calibration matrices.
#         points3D (np.ndarray): [P, 3] 3D point coordinates (NaN for filtered points if original_num_points provided).
#     """

#     num_images = len(reconstruction.images)
    
#     # FIX: Return NaN-filled array maintaining original size
#     if original_num_points is not None:
#         # Create array for all original points, filled with NaN
#         points3D = np.full((original_num_points, 3), np.nan)
        
#         # Fill in only the points that survived BA
#         # COLMAP point IDs are 1-based and correspond to original indices
#         for point3D_id, point in reconstruction.points3D.items():
#             idx = point3D_id - 1  # Convert to 0-based index
#             if idx < original_num_points:
#                 points3D[idx] = point.xyz
#     else:
#         # Original behavior: compact array (for backward compatibility)
#         point_ids = sorted(reconstruction.point3D_ids())
#         num_points = len(point_ids)
#         points3D = np.zeros((num_points, 3))
        
#         for idx, point3D_id in enumerate(point_ids):
#             points3D[idx] = reconstruction.points3D[point3D_id].xyz
