...
#     :return: errors [m,n]
#     """
# 	# print('Ps', Ps.shape, Ps.min(), Ps.max())
...
# 	# print('errors', errors.shape, errors.min(), errors.max())
# 	# print('visible_points', visible_points.shape, visible_points.min(), visible_points.max())
# 	# print(np.unique(visible_points, return_counts=True))
# 	# exit()
# 	# print('errors1', errors.shape, errors.min(), errors.max())
# 	# print('errors1', np.unique(errors))
# 	errors[~visible_points] = np.nan
# 	# print('errors', errors.shape, errors.min(), errors.max())
# 	# print('errors2', np.unique(errors))
# 	# exit()
# 	return errors
...
# 	"""
#     :param Ps: [m,3,4]
#     :param Xs: [n,3] or [n,4] (may contain NaN for filtered points)
#     :param xs: [m,n,2]
#     :return: errors [m,n]
#     """
# 	m, n, d = xs.shape
...
# 	# ========== FIX START - return nan filled arrays ==========
# 	# Filter out NaN points (points removed by bundle adjustment)
# 	valid_point_mask = ~np.isnan(Xs[:, 0])
# 	num_valid = valid_point_mask.sum()
	
# 	if num_valid < Xs.shape[0]:
# 		# Some points were filtered by BA
# 		print(f"INFO: Filtering {Xs.shape[0] - num_valid} NaN points from reprojection computation")
# 		Xs = Xs[valid_point_mask]
# 		xs = xs[:, valid_point_mask, :]
# 		if visible_points is not None:
# 			visible_points = visible_points[:, valid_point_mask]
# 		n = num_valid  # Update n to filtered count
# 	# ========== FIX END ==========
